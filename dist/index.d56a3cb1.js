...
function getCurrencies(country) {
    if (country.length === 2) return `and you can pay with ${country[0].name} and ${country[1].name}'s`;
    else return `and you can pay with ${country[0].name}'s`;
}
function getLanguages(country) {
    let string;
    if (country.length === 1) return `They speak ${country[0].name}`;
    else if (country.length > 1) {
        string = `They speak ${country[0].name}`;
        console.log(string);
        for(let i = 1; i < country.length - 1; i++)string += `, ${country[i].name}`;
